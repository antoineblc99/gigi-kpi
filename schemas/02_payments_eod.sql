...
create table if not exists fact_eod_closeuse (
  ...
  calls_planifies int default 0,
  calls_recus     int default 0,
  follow_ups      int default 0,
  acomptes        int default 0,
  ventes_setting  int default 0,
  ventes_vsl      int default 0,
  cash_contracte  numeric default 0,
  cash_collecte   numeric default 0,
  qualif_lead     int,
  debrief         text,
  fathom_urls     text[],
  raw             jsonb,
...
-- Payment source extensions (supports Stripe legacy + Whop CSV imports)
...
alter table fact_payment add column if not exists source           text;
