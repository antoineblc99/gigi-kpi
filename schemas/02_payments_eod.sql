...
-- 1. EOD closeuse (Tally form, manual end-of-day report)
create table if not exists fact_eod_closeuse (
  id              bigserial primary key,
  network_id      text unique,                 -- Tally response id (idempotent upsert)
...
create index if not exists fact_eod_closeuse_date_idx on fact_eod_closeuse (submit_date desc);
create index if not exists fact_eod_closeuse_closer_idx on fact_eod_closeuse (closer_id, submit_date desc);

-- 2. Extend dim_lead for fbclid (UTM tracking end-to-end)
alter table dim_lead add column if not exists fbclid text;

-- 3. Extend fact_payment for Stripe sync
...
create index if not exists fact_payment_paid_idx  on fact_payment (paid_at desc);
create index if not exists fact_payment_email_idx on fact_payment (lower(customer_email));
